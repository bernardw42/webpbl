    ...
    return (
        <div className="flex flex-col justify-center items-center">
            <div className="bg-white w-full flex justify-center items-center pb-[150px] pt-[150px]" ref={textRef}>
                <h1 className={`text-center max-w-7xl text-red-500 font-bold text-3xl max-md:text-2xl max-md:leading-[50px] px-8 transition-all duration-[1500ms] leading-[60px] opacity-0 ${textInView ? 'opacity-100' : ''}`}>We offer best logistic solutions to improve efficiency on custom production and distribution throughout Indonesia.</h1>
            </div>
            <div ref={ref} className="bg-[#F9F9F9] w-full flex flex-wrap justify-center items-center gap-x-[100px] gap-y-[70px] pb-[200px] pt-[200px] max-lg:pb-[150px] max-lg:pt-[150px] px-8">
                    ...
                    {state => (
                        <div className="flex flex-col gap-y-[40px]">
                            <div className={`flex items-center transform gap-x-[20px] transition-all duration-[1500ms] ${state === 'entered' ? 'translate-x-0 opacity-100' : 'translate-x-[150px] opacity-0'}`}>
                                ...
                                <h1 className="text-blue-700 font-semibold text-xl max-md:text-lg">One Stop Solution Logistics Service <br></br> (Supply Chain Management)</h1>
                            </div>
                            <div className={`flex items-center transform gap-x-[20px] transition-all duration-[1500ms] ${state === 'entered' ? 'translate-x-0 opacity-100' : 'translate-x-[150px] opacity-0'}`}>
                                <Image src={img3} alt="map" className='w-[70px] max-md:w-[60px]' />
                                <h1 className="text-blue-700 font-semibold text-xl max-md:text-lg">Warehouse Management</h1>
                            </div>
                            <div className={`flex items-center transform gap-x-[20px] transition-all duration-[1500ms] ${state === 'entered' ? 'translate-x-0 opacity-100' : 'translate-x-[150px] opacity-0'}`}>
                                <Image src={img6} alt="map" className='w-[70px] max-md:w-[60px]' />
                                <h1 className="text-blue-700 font-semibold text-xl max-md:text-lg">Freight Forwarding Service</h1>
                            </div>
                            <div className={`flex items-center transform gap-x-[20px] transition-all duration-[1500ms] ${state === 'entered' ? 'translate-x-0 opacity-100' : 'translate-x-[150px] opacity-0'}`}>
                                <Image src={img7} alt="map" className='w-[70px] max-md:w-[60px]' />
                                <h1 className="text-blue-700 font-semibold text-xl max-md:text-lg">Experiencing Manpower</h1>
                            </div>
                        </div>
                    )}
                </Transition>
